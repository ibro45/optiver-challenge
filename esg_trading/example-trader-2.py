...
Quoting an instrument means sending both bids and 
asks at the same time and making these publicly visible in the order book,
...
def trade_cycle(e: Exchange):
    ...
    if basket_book and basket_book.bids and basket_book.asks:
        ...
        if new_ask_price - new_bid_price > 0.01:
            e.insert_order(BASKET_INSTRUMENT_ID, price=new_bid_price, volume=1, side=SIDE_BID, order_type=ORDER_TYPE_LIMIT)
            e.insert_order(BASKET_INSTRUMENT_ID, price=new_ask_price, volume=1, side=SIDE_ASK, order_type=ORDER_TYPE_LIMIT)
        else:
