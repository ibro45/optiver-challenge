...
def trade_cycle(e: Exchange):
    ...
    if basket_book and basket_book.bids and basket_book.asks:
        ...
        if random.random() < 0.5:
            e.insert_order(BASKET_INSTRUMENT_ID, price=basket_book.asks[0].price, volume=1, side=SIDE_BID, order_type=ORDER_TYPE_IOC)
        else:
            e.insert_order(BASKET_INSTRUMENT_ID, price=basket_book.bids[0].price, volume=1, side=SIDE_ASK, order_type=ORDER_TYPE_IOC)
    else:
